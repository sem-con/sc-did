class DidsController < ApplicationController
    ...
    def show
        ...
        @item = Store.find_by_did("did:web:id.data-container.net:r:" + did_.to_s)
        if @item.nil?
        ...
        signing_key = Ed25519::SigningKey.new(Base58.base58_to_binary(@item.did_private_key)) rescue nil?
        if signing_key.nil?
            render json: {"error": "invalid DID"}, 
                   status: 404
            return
...
"id": "did:web:id.data-container.net:r:' + did_ + '",
"publicKey": [{
    "id": "did:web:id.data-container.net:r:' + did_ + '#key-1",
    "type": "Ed25519VerificationKey2018",
    "controller": "id.data-container.net:r:' + did_ + '",
    "expires": "' + (Time.now+2.years).utc.iso8601 + '",
    "publicKeyBase58": "' + Base58.binary_to_base58(verify_key.to_bytes) + '"
}],
"service": ' + @item.item.to_s + '
}')
